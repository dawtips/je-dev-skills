...
def render_subagent(subagent: dict) -> str:
    ...
    return "\n".join([
        ...
        f"description: Handles the {slugify(agent_id)} step for the scaffolded workflow",
        f"model: {model}",
        f"tools: {tools}",
        "---",
        "",
...
def render_hook(rubric: dict) -> str:
    ...
    gate = _parse_positive_int(rubric.get("gate"), f"rubric {name} gate")
    return "\n".join([
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "",
        f"# AUTO-GENERATED by agent-build-scaffold for rubric: {name}. Exit 0 = pass, exit 2 = block.",
        'PROJECT_DIR="${CLAUDE_PROJECT_DIR:-$PWD}"',
        f'SCORE_FILE="$PROJECT_DIR/.agent-build-state/{name}.score"',
...
def _parse_positive_int(value, label: str) -> int:
...
def render_entry_command(bp: dict, workflow_name: str) -> str:
    ...
    if rubrics:
        ...
        for rubric in rubrics:
            ...
            gate = _parse_positive_int(rubric.get("gate"), f"rubric {name} gate")
            lines.append(
                f"- {name}: write score `.agent-build-state/{name}.score`, "
                f"then run `.claude/hooks/{hook_filename(rubric)}` (gate `{gate}`)."
            )
        lines.append("")
